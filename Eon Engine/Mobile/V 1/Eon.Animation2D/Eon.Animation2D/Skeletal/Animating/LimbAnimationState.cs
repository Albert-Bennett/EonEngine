...
namespace Eon.Animation2D.Skeletal.Animating
{
    ...
    public struct LimbAnimationState
    {
        ...
        {
            if (Animation.LimbFrames.Length > 1)
            {
                ...
                while (timePast.TotalSeconds > currentLenght)
                {
                    ...
                    nextIndex = (nextIndex + 1) % Animation.LimbFrames.Length;

                    int frame1 = Animation[currentIndex].FrameNumber;
                    int frame2 = Animation[nextIndex].FrameNumber;
                    int difference = frame2 > frame1 ? frame2 - frame1 : (Animation.LimbFrames.Length - frame1) + frame2;

                    currentLenght = difference / (float)Animation.FrameRate;
                }
        ...
        public void Reset()
        {
            ...
            if (Animation.Count > 1)
            {
                currentLenght = (Animation[1].FrameNumber -
                    Animation[0].FrameNumber) / Animation.FrameRate;
