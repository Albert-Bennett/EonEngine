    ...
    public class GamePad : BaseInputItem
    {
        ...
        protected override void Update()
        {
            ...
            if (!currentState.IsConnected)
                InputManager.CurrentGamePadDisconnected(this.index);
        ...
        internal bool IsPadTriggerStroked(TriggerIndex index)
        {
            return (Triggers[(int)index] > 0.0f && prevTriggers[(int)index] <= 0.0f);
        }

        internal bool IsPadTriggerPressed(TriggerIndex index)
        {
            return triggers[(int)index] > 0.0f;
        }

        internal bool IsPadTriggerReleased(TriggerIndex index)
        {
            return triggers[(int)index] == 0.0f;
        }
