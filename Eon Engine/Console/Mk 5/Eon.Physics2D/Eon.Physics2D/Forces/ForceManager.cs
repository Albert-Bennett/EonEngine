...
namespace Eon.Physics2D.Forces
{
    ...
    /// </summary>
    internal class ForceManager : EngineModule, IUpdate
    {
        ...
        static List<Particle> accumulators = new List<Particle>();
        ...
        internal static void Add(Particle accumulator)
        {
        ...
        public void _Update()
        {
            ...
            for (int i = 0; i < forces.Count; i++)
                for (int j = 0; j < accumulators.Count; j++)
                    if (!accumulators[j].IsStatic)
                        forces[i].ApplyForce(accumulators[j]);

            for (int i = 0; i < accumulators.Count; i++)
                accumulators[i].Update();
        }
        ...
        internal static void Remove(Particle accumulator)
        {
