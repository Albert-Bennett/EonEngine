...
namespace Eon.Animation2D.Skeletal
{
    ...
    /// </summary>
    public sealed class Skeleton : ObjectComponent
    {
        #region Varibles
        ...
        /// </summary>
        /// <param name="frames">The individual Limb animations to be applied.</param>
        internal void SetLimbTransformation(LimbAnimationState[] frames)
        {
            SetLimbTransforms("", frames);
        }

        void SetLimbTransforms(string parentName, LimbAnimationState[] frames)
        {
            for (int i = 0; i < limbs.Count; i++)
                if (limbs[i].ParentLimb == parentName)
                {
                    ...
                    if (transform != null)
                    {
                        limbs[i].FrameTransform = transform;
                    ...
                    else
                    {
                        ...
                        limbs[i].SetTransform(Transformation.Compose(
                            parentTransform, limbs[i].FrameTransform));
                    }

                    SetLimbTransforms(limbs[i].Name, frames);
                }
        }

        int GetLimbIndex(string limbName)
        {
            int idx = 0;
            bool found = false;

            while (idx < limbs.Count && !found)
            {
                if (limbs[idx].Name == limbName)
                    found = true;
                else
                    idx++;
            }
        ...
        Transformation GetParentTransform(string limbName)
        {
            string parentName = "";
            ...
            while (i < limbs.Count && !found)
            {
                if (limbs[i].Name == limbName)
                {
                    parentName = limbs[i].Name;
                    found = true;
                }
            ...
            return FindTransform(parentName);
        }
