...
{
    /// <summary>
    /// Used to define an IBounding that takes the form of a box.
    /// </summary>
    public struct EonBoundingBox : IEquatable<EonBoundingBox>, IBounding
    {
        #region Properties
        ...
        public EonBoundingBox(Vector3 min, Vector3 max)
        {
            Min = min;
            Max = max;
        }
        ...
        /// <param name="min">Minimum values for this.</param>
        /// <param name="max">Maximum values for this.</param>
        public EonBoundingBox(float min, float max) : this(new Vector3(min), new Vector3(max)) { }
        ...
        public EonBoundingBox CreateFromValueString(string value)
        {
            value = value.Replace("(", "");
            value = value.Replace(")", "");
            value = value.Replace("{", "");
            ...
            if (values.Length == 6)
                return new EonBoundingBox(new Vector3(Convert.ToSingle(values[0]), Convert.ToSingle(values[1]),
                    Convert.ToSingle(values[2])), new Vector3(Convert.ToSingle(values[3]), Convert.ToSingle(values[4]),
                    Convert.ToSingle(values[5])));

            return new EonBoundingBox(0, 0);
        }
        ...
        /// <returns>The generated BoundingBox.</returns>
        public EonBoundingBox CreateFrom(IList<Vector3> positions)
        {
            ...
            if (min.X == max.X)
            {
                min.X -= EonMathsHelper.Epsilon;
                max.X += EonMathsHelper.Epsilon;
            }
            if (min.Y == max.Y)
            {
                min.Y -= EonMathsHelper.Epsilon;
                max.Y += EonMathsHelper.Epsilon;
            }
            if (min.Z == max.Z)
            {
                min.Z -= EonMathsHelper.Epsilon;
                max.Z += EonMathsHelper.Epsilon;
            }

            return new EonBoundingBox(min, max);
        }
        ...
        public void Transform(ref Matrix transform)
        {
            ...
            Matrix abs;
            EonMathsHelper.Abs(ref transform, out abs);
        ...
        public void CreateMerged(EonBoundingBox other)
        {
        ...
        public EonBoundingSphere ConvertToBoundingSphere()
        {
            float radius = ((Max - Min) / 2).Length();
        ...
        {
            return obj is EonBoundingBox ? Equals((EonBoundingBox)obj) : base.Equals(obj);
        }
        ...
        /// <returns>The Result.</returns>
        public bool Equals(EonBoundingBox other)
        {
            return this == other;
        }
        ...
        /// <returns>The result of the check.</returns>
        public static bool operator ==(EonBoundingBox box1, EonBoundingBox box2)
        {
