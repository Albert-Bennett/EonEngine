...
namespace Eon.PostProcessing.Effects
{
    ...
    public sealed class DOFEffect : PostProcess
    {
        ...
        public DOFEffect(float blurStart, float blurEnd, float blurAmount)
            : base("DOFPP", 1, "Render3DFramework")
        {
            try
            {
            ...
            effect = Common.ContentManager.Load<Effect>("Eon/Shaders/PostProcessing/DOF");
            blur = Common.ContentManager.Load<Effect>("Eon/Shaders/PostProcessing/GausianBlur");
        }
        ...
        void RenderBlur()
        {
            ...
            DrawWithEffect(TextureBuffer.GetTexture("Scene"), blurred, blur);
        ...
        {
            blurred = new RenderTarget2D(Common.Device, width, height);
