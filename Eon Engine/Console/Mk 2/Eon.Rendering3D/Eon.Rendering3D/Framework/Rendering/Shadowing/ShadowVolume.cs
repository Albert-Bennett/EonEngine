...
namespace Eon.Rendering3D.Framework.Rendering.Shadowing
{
    ...
    internal sealed class ShadowVolume
    {
        ...
        public void Build(ModelComponent model, ICastShadows light)
        {
            for (int i = 0; i < model.Meshes.Count; i++)
                if (model.Meshes[i].RenderType != RenderTypes.Transparency)
                    if (light.Shadows(model.Meshes[i].BoundingSphere))
                        Construct(model.Meshes[i], light.Direction);
        }
