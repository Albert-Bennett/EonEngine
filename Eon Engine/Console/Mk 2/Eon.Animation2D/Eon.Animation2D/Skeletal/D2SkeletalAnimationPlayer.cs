...
namespace Eon.Animation2D.Skeletal
{
    ...
    /// </summary>
    public sealed class D2SkeletalAnimationPlayer : ObjectComponent, IUpdate
    {
        ...
        {
            AddAnimation(XmlHelper.Deserialize<D2SkeletalAnimation>(animationFilepath, ".SkelAni", true));
        }
        ...
        public void AddAnimations(string[] animationFilepaths)
        {
            for (int i = 0; i < animationFilepaths.Length; i++)
                AddAnimation(XmlHelper.Deserialize<D2SkeletalAnimation>(animationFilepaths[i], ".SkelAni", true));
        }
