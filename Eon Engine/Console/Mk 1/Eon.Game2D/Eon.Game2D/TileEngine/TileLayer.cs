...
namespace Eon.Game2D.TileEngine
{
    ...
    public sealed class TileLayer
    {
        ...
        void CalculateChunks()
        {
            ...
            int chunkRowLimit = (int)(Common.ScreenResolution.Y / size.Y);
            int chunkColLimit = (int)(Common.ScreenResolution.X / size.X);
            ...
            int totalRows = tileDeff.Tiles.Count;
            int remainingRows = totalRows - chunkRowLimit;
            int maxColumns = GetMaxColumns();

            Vector2 initialPos = new Vector2(0, Common.ScreenResolution.Y - size.Y) + offset;
        ...
        int GetMaxColumns()
        {
            ...
            for (int i = 0; i < tileDeff.Tiles.Count; i++)
                if (tileDeff.Tiles[i].Count > max)
                    max = tileDeff.Tiles[i].Count;
