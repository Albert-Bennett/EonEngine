...
namespace Eon
{
    ...
    public class GameObject : IID
    {
        ...
        List<IPriorityComponent> priorities = new List<IPriorityComponent>();
        ...
        void InitializeComponents()
        {
            if (unInit.Count > 0)
            {
                foreach (ObjectComponent comp in unInit)
                    comp._Initialize();
        ...
        void UpdateComponents()
        {
            ...
            for (int i = 0; i < updateComps.Count; i++)
                if (((ObjectComponent)updateComps[i]).Enabled)
