...
namespace Eon.Game2D.TileEngine
{
    ...
    public sealed class TileLayer
    {
        ...
        internal void Initialize()
        {
            ...
            tileSheet = Common.ContentBuilder.Load<Texture2D>(tileDeff.TileSheetFilepath);

            if (tileDeff.NormalMapFilepath != "None")
                normalMap = Common.ContentBuilder.Load<Texture2D>(tileDeff.NormalMapFilepath);

            if (tileDeff.DistortionMapFilepath != "None")
                distortionMap = Common.ContentBuilder.Load<Texture2D>(tileDeff.DistortionMapFilepath);
