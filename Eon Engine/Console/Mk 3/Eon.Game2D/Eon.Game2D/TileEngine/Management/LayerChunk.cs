...
namespace Eon.Game2D.TileEngine.Management
{
    ...
    public sealed class LayerChunk : IDrawItem
    {
        ...
        {
            int total = attachedTo.LayerDeff.Tiles.Length - startRow;
        ...
        {
            int remainingColumns = attachedTo.LayerDeff.Tiles.GetLength(1) - startCol;
        ...
        bool ShouldRender()
        {
            if (CameraManager2D.CurrentCamera != null)
                return CameraManager2D.CurrentCamera.IsInView(bounds);
        ...
        bool DrawTileColour(Tile tile)
        {
            if (CameraManager2D.CurrentCamera != null)
            {
                if (CameraManager2D.CurrentCamera.IsInView(tile.Bounds))
                {
