...
float4 RectifyOpaqueData(float4 sceneOpaque, float4 opaque, float alpha)
{
	half3 sceneNorm = DecodeNormal(sceneOpaque.rg);
	half3 norm = DecodeNormal(opaque.rg);

	sceneNorm += norm *  alpha;
	sceneNorm = normalize(sceneNorm);

	float4 output;
	output.rg = EncodeNormal(sceneNorm);
	output.ba = sceneOpaque.ba + (opaque.ba * alpha);
...
{
	float alpha = peelColour.a;

	[branch]
	if(alpha < 1.0f)
	{
		outOpaque = RectifyOpaqueData(opaque, peelOpaque, alpha);
		outColour = float4(colour + (peelColour.rgb * (1.0f - alpha)), 1.0f);
	}
	else
	{
