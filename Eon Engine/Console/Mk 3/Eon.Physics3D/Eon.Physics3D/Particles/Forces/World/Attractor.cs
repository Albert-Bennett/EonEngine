...
namespace Eon.Physics3D.Particles.Forces.World
{
    ...
    public sealed class Attractor : IForce
    {
        ...
        public void ApplyForce(object accumulator)
        {
            ...
            Vector3 direction = position - acc.Position;

            if (range > 0)
            {
                float dist = Vector3.Distance(position, acc.Position);
                ...
                if (dist > range)
                {
                    ...
                    acc.AccumulateForces(direction * f);
                }
            }
            else
                acc.AccumulateForces(direction * force);
        }
