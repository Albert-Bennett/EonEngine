...
namespace Eon.Engine.Input
{
    ...
    public class Keyboard : BaseInputItem
    {
        ...
        internal bool IsKeyStroked(Keys key)
        {
        ...
        internal string[] GetKeyInput()
        {
            List<string> value = new List<string>();

            Microsoft.Xna.Framework.Input.Keys[] keys = currentState.GetPressedKeys();
            Microsoft.Xna.Framework.Input.Keys[] prevKeys = prevState.GetPressedKeys();

            for (int i = 0; i < keys.Length; i++)
                if (prevKeys == null && keys.Length > 0)
                    value.Add(GetKeyValue(keys[i]));
                else if (prevKeys.Length > 0)
                {
                    if (i > prevKeys.Length - 1)
                        value.Add(GetKeyValue(keys[i]));
        ...
        static string GetKeyValue(Microsoft.Xna.Framework.Input.Keys key)
        {
            string value = "";
            string text = Enum.GetName(typeof(Microsoft.Xna.Framework.Input.Keys), key);
