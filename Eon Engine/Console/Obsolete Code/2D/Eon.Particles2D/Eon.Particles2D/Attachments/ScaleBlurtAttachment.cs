...
namespace Eon.Particles2D.Attachments
{
    ...
    public class ScaleBlurtAttachment : IAttachment
    {
        ...
        BlurtState GenerateBlurt()
        {
            ...
            {
                DeValue = false,
                MaxValue = RandomHelper.GetRandom(minMaxScale, maxMaxScale),
                MinValue = RandomHelper.GetRandom(minMinScale, maxMinScale),
                Decay = RandomHelper.GetRandom(minDecay, maxDecay),
                Value = RandomHelper.GetRandom(minMinScale, maxMaxScale)
            };
        }
