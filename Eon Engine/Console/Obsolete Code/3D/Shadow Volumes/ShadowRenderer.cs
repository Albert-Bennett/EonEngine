...
namespace Eon.Rendering3D.Framework.Rendering.Shadowing
{
    ...
    internal sealed class ShadowRenderer
    {
        ...
        {
            LightManager.GetShadowCasters(out shadowCasters);

            List<MeshPart> models = null;
            ModelManager.GetVisableModels(out models);

            for (int i = 0; i < shadowCasters.Count; i++)
                for (int j = 0; j < models.Count; j++)
                    if (shadowCasters[i].Shadows(models[j].BoundingSphere))
                    {
                        ShadowVolume shadow = new ShadowVolume();
                        shadow.Build(models[j], shadowCasters[i]);
        ...
        void RenderShadowVolumes()
        {
            if (shadows.Count > 0)
            {
                ...
                for (int i = 0; i < shadowCasters.Count; i++)
                {
                    ...
                    shadowEffect.Parameters["DepthMap"].SetValue(Framework.TextureBuffer.GetTexture("DepthMap"));
                ...
                Framework.TextureBuffer.SetBuffer("Shadows", target);
            }
        }
